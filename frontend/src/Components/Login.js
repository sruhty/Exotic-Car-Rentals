...
import {checkingAuth, CheckEmail, forgotThePassword} from "../Services/actions/auth";
...
class Login extends React.Component{
    ...
    onLogin = () => {
        ...
        this.setState({loading:true},async ()=>{
            const response = await CheckEmail(send);
            if (response && response.data) {
                const token = response.data.token;
                localStorage.setItem("token", token);
                await checkingAuth(token);
    ...
    forgetPassword =  () =>{
      ...
      if(!data.email){
          ...
          this.setState({loading:true},async ()=>{
              const res= await forgotThePassword(data.email);
              if(res && res.success){
